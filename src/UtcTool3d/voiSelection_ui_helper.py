...
from src.UtcTool3d.philips3dRf import philipsRfParser3d
...
class VoiSelectionGUI(Ui_constructVoi, QWidget):
    def __init__(self):
        ...
        self.imgDataStruct, self.imgInfoStruct = None, None
        self.data4dImg = None
        self.preScData4dImg = None
        self.rfData4dImg = None
        self.samplingFrequency = None
        self.centerFrequency = None
        self.transducerFreqBand = None
        self.analysisFreqBand = None
        self.imagePath = None
    ...
    def saveToMat(self):
        outDict = {
            "Sampling_Frequency": self.samplingFrequency,
            "Center_Frequency": self.centerFrequency,
            "Transducer_Frequency_Band": np.array(self.transducerFreqBand),
            "Analysis_Frequency_Band": np.array(self.analysisFreqBand),
            "Bmode": self.preScData4dImg.astype(np.uint8),
            "RF": self.rfData4dImg,
            "SC_Bmode": self.data4dImg.astype(np.uint8),
        }
    ...
    def updateAdvancedRoiEditButtons(self):
        if not self.saveVoiButton.isHidden() or not np.amax(self.maskCoverImg): # if 3D VOI interpolation is complete
