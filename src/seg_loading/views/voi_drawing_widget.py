...
from engines.qus.quantus.gui.seg_loading.ui.voi_drawing_ui import Ui_voi_drawer
...
class VoiDrawingWidget(QWidget, BaseViewMixin):
    """
    ...
    seg_file_selected = pyqtSignal(dict)  # {'seg_path': str, 'seg_type': str}
    ...
    def __init__(self, image_data: UltrasoundRfImage, parent: Optional[QWidget] = None):
        ...
        self._pix_data = image_data.sc_bmode  # 4D numpy array (x,y,z,t)
        if self._pix_data.ndim != 4:
            assert self._pix_data.ndim == 3
            self._pix_data = self._pix_data[:, :, :, np.newaxis]
    ...
    def _on_alpha_changed(self):
        ...
        self._roi_masks_overlap[self._roi_masks_overlap[:,:,:,0] > 0] = [255, 0, 0, int(alpha)]
        self._refresh_frames()
    ...
    def _on_restart_voi(self):
        ...
        # Update UI
        self._ui.alpha_spin_box.setValue(125); self._ui.alpha_status.setValue(125)
        self._hide_widget_lists([self._voi_decision_widgets, self._voi_alpha_widgets])
    ...
    def _save_voi(self):
        ...
        affine = np.eye(4)
        for i, res in enumerate([self._image_data.sc_coronal_res, self._image_data.sc_lateral_res, self._image_data.sc_axial_res]):
            affine[i, i] = res
    ...
    def _set_interp_loading(self, loading_state: bool) -> None:
        ...
        if loading_state:
            ...
            self._show_widget_lists([self._voi_alpha_widgets, self._voi_decision_widgets])
